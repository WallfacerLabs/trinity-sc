...
contract("Gas compensation tests", async accounts => {
   ...
   -> Expect composite debt = (54321.123456789 + 200) = 54521.123456789 TRI */
	...
	it("gas compensation from pool-offset liquidations. All collateral paid as compensation", async () => {
		...
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(2, 18)),
			extraTRIAmount: A_totalDebtERC20,
			extraParams: { from: dennis },
		})
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(2, 18)),
			extraTRIAmount: B_totalDebtERC20.add(C_totalDebtERC20),
			extraParams: { from: erin },
		})

		// console.log(((await community.GRVTSupplyCap()).toString()))

		// D, E each provide TRI to SP
		...
		const TRIinSP_0ERC20 = await stabilityPool.getTotalDebtTokenDeposits()
    ...
    -> Expect 0.5% of collaterall to be sent to liquidator, as gas compensation */
		...
		// Check SP TRI has decreased due to the liquidation
		const TRIinSP_AERC20 = await stabilityPool.getTotalDebtTokenDeposits()
		assert.isTrue(TRIinSP_AERC20.lte(TRIinSP_0ERC20))
    ...
    -> Expect 0.5% of collaterall to be sent to liquidator, as gas compensation */
		...
		// Check SP TRI has decreased due to the liquidation of B
		const TRIinSP_BERC20 = await stabilityPool.getTotalDebtTokenDeposits()

		assert.isTrue(TRIinSP_BERC20.lt(TRIinSP_AERC20))
    ...
    -> Expect 0.5% of collaterall to be sent to liquidator, as gas compensation */
		...
		// Check SP TRI has decreased due to the liquidation of C
		assert.isTrue((await stabilityPool.getTotalDebtTokenDeposits()).lt(TRIinSP_BERC20))
	...
	it("gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder", async () => {
		...
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(60, 18)),
			extraTRIAmount: dec(600, 18),
			extraParams: { from: carol },
		})
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(80, 18)),
			extraTRIAmount: dec(1, 23),
			extraParams: { from: dennis },
		})
		await openVessel({
			asset: erc20.address,
		...
		// D, E each provide 10000 TRI to SP
    ...
    and (1 - 0.05000025000125001) = 0.94999974999875 ETH remainder liquidated */
		...
		// Check SP TRI has decreased due to the liquidation of A
		const TRIinSP_AERC20 = await stabilityPool.getTotalDebtTokenDeposits()
		assert.isTrue(TRIinSP_AERC20.lte(TRIinSP_0ERC20))
    ...
    and (15 - 0.666666666666666666) ETH remainder liquidated */
		...
		// Check SP TRI has decreased due to the liquidation of B
		assert.isTrue((await stabilityPool.getTotalDebtTokenDeposits()).lt(TRIinSP_AERC20))
   ...
   and (37.5 - 0.1875 ETH) ETH remainder liquidated */
	...
	it("gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt", async () => {
		...
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(2, 18)),
			extraTRIAmount: B_totalDebtERC20,
			extraParams: { from: erin },
		})
    ...
    */
		...
		// Check TRI in SP has decreased
		assert.isTrue((await stabilityPool.getTotalDebtTokenDeposits()).lt(TRIinSP_0ERC20))
	...
	it("liquidateVessels(): full redistribution. Compensates the correct amount, and liquidates the remainder", async () => {
		...
		const TRIinDefaultPool_0ERC20 = await defaultPool.getDebtTokenBalance(erc20.address)
    ...
    */
		...
		// Check TRI in DefaultPool has decreased
		assert.isTrue((await defaultPool.getDebtTokenBalance(erc20.address)).gt(TRIinDefaultPool_0ERC20))
    ...
    */
		...
		// Expect liquidatedDebt = 51 + 190 + 1025 + 13510 = 14646 TRI
