...
contract("VesselManager - in Recovery Mode - back to normal mode in 1 tx", async accounts => {
	...
	context("Batch liquidations", () => {
		...
		it("Stability Pool profit matches", async () => {
			...
			const spTRIBefore_Asset = await stabilityPool.getTotalDebtTokenDeposits()
			...
			const realGainInTRI_Asset = spEthAfter_Asset
				.sub(spEthBefore_Asset)
				.mul(price)
				.div(mv._1e18BN)
				.sub(spTRIBefore_Asset.sub(spGRVTfter_Asset))
			...
			assert.equal(
				spTRIBefore_Asset.sub(spGRVTfter_Asset).toString(),
				A_totalDebt_Asset.toString(),
				"Stability Pool TRI doesn’t match"
			)
			assert.equal(
				realGainInTRI_Asset.toString(),
				expectedGainInTRI_Asset.toString(),
