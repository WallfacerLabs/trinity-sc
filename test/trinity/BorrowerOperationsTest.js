...
contract("BorrowerOperations", async accounts => {
	...
	describe("BorrowerOperations Mechanisms", async () => {
		...
		it("addColl(): active Vessel: applies pending rewards and updates user's L_ETH, L_TRIDebt snapshots", async () => {
			// --- SETUP ---

			const { collateral: aliceCollBeforeAsset, totalDebt: aliceDebtBeforeAsset } = await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(15000, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: alice },
			})
			...
			// price drops to 1ETH:100TRI, reducing Carol's ICR below MCR
			...
			const L_TRIDebt_Asset = await vesselManager.L_Debts(erc20.address)
			...
			const alice_TRIDebtRewardSnapshot_Before_Asset = alice_rewardSnapshot_Before_Asset[1]
			...
			const bob_TRIDebtRewardSnapshot_Before_Asset = bob_rewardSnapshot_Before_Asset[1]

			assert.equal(alice_ETHrewardSnapshot_Before_Asset, 0)
			assert.equal(alice_TRIDebtRewardSnapshot_Before_Asset, 0)
			assert.equal(bob_ETHrewardSnapshot_Before_Asset, 0)
			assert.equal(bob_TRIDebtRewardSnapshot_Before_Asset, 0)
			...
			const alicePendingTRIDebtRewardAsset = await vesselManager.getPendingDebtTokenReward(erc20.address, alice)
			const bobPendingTRIDebtRewardAsset = await vesselManager.getPendingDebtTokenReward(erc20.address, bob)

			for (reward of [
				alicePendingETHRewardAsset,
				bobPendingETHRewardAsset,
				alicePendingTRIDebtRewardAsset,
				bobPendingTRIDebtRewardAsset,
			]) {
			...
			assert.isTrue(aliceNewDebt_Asset.eq(aliceDebtBeforeAsset.add(alicePendingTRIDebtRewardAsset)))
			assert.isTrue(bobNewColl_Asset.eq(bobCollBeforeAsset.add(bobPendingETHRewardAsset).add(bobTopUp)))
			assert.isTrue(bobNewDebt_Asset.eq(bobDebtBeforeAsset.add(bobPendingTRIDebtRewardAsset)))

			/* Check that both Alice and Bob's snapshots of the rewards-per-unit-staked metrics should be updated
       to the latest values of L_ETH and L_TRIDebt */
			...
			const alice_TRIDebtRewardSnapshot_After_Asset = alice_rewardSnapshot_After_Asset[1]
			...
			const bob_TRIDebtRewardSnapshot_After_Asset = bob_rewardSnapshot_After_Asset[1]

			assert.isAtMost(th.getDifference(alice_ETHrewardSnapshot_After_Asset, L_Asset), 100)
			assert.isAtMost(th.getDifference(alice_TRIDebtRewardSnapshot_After_Asset, L_TRIDebt_Asset), 100)
			assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot_After_Asset, L_Asset), 100)
			assert.isAtMost(th.getDifference(bob_TRIDebtRewardSnapshot_After_Asset, L_TRIDebt_Asset), 100)
		})
		...
		it("withdrawColl(): reverts if system is in Recovery Mode", async () => {
			...
			// price drops to 1ETH:150TRI, reducing TCR below 150%
		...
		it("withdrawColl(): applies pending rewards and updates user's L_ETH, L_TRIDebt snapshots", async () => {
			// --- SETUP ---
			...
			// close Carol's Vessel, liquidating her 1 ether and 180TRI.
			await vesselManagerOperations.liquidate(erc20.address, carol, { from: owner })
       ...
       to the latest values of L_ETH and L_TRIDebt */
		...
		it("withdrawDebtTokens(): reverts when withdrawal would leave vessel with ICR < MCR", async () => {
			...
			const TRIwithdrawal = 1 // withdraw 1 wei TRI

			await assertRevert(
				borrowerOperations.withdrawDebtTokens(erc20.address, TRIwithdrawal, alice, alice, {
					from: alice,
				}),
				"BorrowerOps: An operation that would result in ICR < MCR is not permitted"
			)
		...
		/* it.skip("withdrawDebtTokens(): decays a non-zero base rate", async () => {
			...
			const A_TRIBal = await debtToken.balanceOf(A)
		...
		it("withdrawDebtTokens(): reverts if fee exceeds max fee percentage", async () => {
			...
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(70, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: C },
			})
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(80, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: D },
			})
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(180, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: E },
			})
		...
		if (!withProxy) {
			...
			it("withdrawDebtTokens(): borrowing at non-zero base records the (drawn debt + fee) on the Vessel struct", async () => {
				...
				const emittedFee_Asset = toBN(th.getTRIFeeFromTRIBorrowingEvent(withdrawalTx_Asset))
		...
		it("withdrawDebtTokens(): borrowing at non-zero base rate sends requested amount to the user", async () => {
			...
			let D_TRIBalanceBefore = await debtToken.balanceOf(D)

			// D withdraws TRI
			const D_TRIRequest = toBN(dec(37, 18))
			await borrowerOperations.withdrawDebtTokens(erc20.address, D_TRIRequest, D, D, {
				from: D,
			})
			...
			//Asset:
			D_TRIBalanceBefore = await debtToken.balanceOf(D)
			await borrowerOperations.withdrawDebtTokens(erc20.address, D_TRIRequest, D, D, {
				from: D,
			})
		...
		it("withdrawDebtTokens(): borrowing at zero fee sends full amount back to user", async () => {
			...
			let D_TRIBalanceAfter = await debtToken.balanceOf(D)
			// Check D's vessel debt == D's TRI balance + liquidation reserve
			assert.isTrue(D_TRIBalanceAfter.eq(D_TRIBalanceBefore.add(D_TRIRequest)))
		})
		...
		it("withdrawDebtTokens(): reverts when calling address does not have active vessel", async () => {
			...
			// Bob successfully withdraws TRI
			...
			// Carol with no active vessel attempts to withdraw TRI
		...
		it("withdrawDebtTokens(): reverts when requested withdrawal amount is zero TRI", async () => {
			await openVessel({
				asset: erc20.address,
				ICR: toBN(dec(2, 18)),
				extraParams: { from: alice },
		...
		it("withdrawDebtTokens(): reverts when system is in Recovery Mode", async () => {
			...
			//Check TRI withdrawal impossible when recoveryMode == true
		...
		it("withdrawDebtTokens(): reverts when withdrawal would bring the vessel's ICR < MCR", async () => {
			...
			// Bob tries to withdraw TRI that would bring his ICR < MCR
		...
		it("withdrawDebtTokens(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR", async () => {
			...
			// Bob attempts to withdraw 1 TRI.
		...
		it("withdrawDebtTokens(): increases the Vessel's TRI debt by the correct amount", async () => {
			await openVessel({
				asset: erc20.address,
				ICR: toBN(dec(2, 18)),
				extraParams: { from: alice },
		...
		it("withdrawDebtTokens(): increases TRI debt in ActivePool by correct amount", async () => {
			await openVessel({
				asset: erc20.address,
			...
			// check before
			const activePool_TRI_Before_Asset = await activePool.getDebtTokenBalance(erc20.address)

			assert.isTrue(activePool_TRI_Before_Asset.eq(aliceDebtBefore_Asset))
			...
			// check after
			const activePool_TRI_After_Asset = await activePool.getDebtTokenBalance(erc20.address)

			th.assertIsApproximatelyEqual(activePool_TRI_After_Asset, activePool_TRI_Before_Asset.add(toBN(dec(10000, 18))))
		})
		...
		it("repayDebtTokens(): reverts when repayment would leave vessel with ICR < MCR", async () => {
			...
			const TRIRepayment = 1 // 1 wei repayment

			await assertRevert(
				borrowerOperations.repayDebtTokens(erc20.address, TRIRepayment, alice, alice, {
					from: alice,
				}),
				"BorrowerOps: An operation that would result in ICR < MCR is not permitted"
			)
		...
		it("repayDebtTokens(): Succeeds when it would leave vessel with net debt >= minimum net debt", async () => {
			// Make the TRI request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
		...
		//withdrawTRI: reduces TRI debt in Vessel
		it("repayDebtTokens(): reduces the Vessel's TRI debt by the correct amount", async () => {
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(2, 18)),
		...
		it("repayDebtTokens(): decreases TRI debt in ActivePool by correct amount", async () => {
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(2, 18)),
			...
			assert.isTrue(activePool_TRI_Before_Asset.gt(toBN("0")))
			...
			th.assertIsApproximatelyEqual(
				activePool_TRI_After_Asset,
				activePool_TRI_Before_Asset.sub(aliceDebtBefore_Asset.div(toBN(10)))
			)
		})
		...
		it("repayDebtTokens(): Reverts if borrower has insufficient TRI balance to cover his debt repayment", async () => {
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(2, 18)),
			...
			// Bob transfers all but 5 of his TRI to Carol
			await debtToken.transfer(C, bobBalBefore.sub(toBN(dec(5, 18))), { from: B })

			//Confirm B's TRI balance has decreased to 5 TRI
			const bobBalAfter = await debtToken.balanceOf(B)

			assert.isTrue(bobBalAfter.eq(toBN(dec(5, 18))))

			// Bob tries to repay 6 TRI
			const repayTRIPromise_B_Asset = borrowerOperations.repayDebtTokens(erc20.address, toBN(dec(6, 18)), B, B, {
				from: B,
			})

			await assertRevert(repayTRIPromise_B_Asset, "Caller doesnt have enough TRI to make repayment")
		})
		...
		it("adjustVessel(): reverts when adjustment would leave vessel with ICR < MCR", async () => {
			...
			await assertRevert(
				borrowerOperations.adjustVessel(erc20.address, 0, 0, TRIRepayment, false, alice, alice, {
					from: alice,
					value: collTopUp,
				}),
				"BorrowerOps: An operation that would result in ICR < MCR is not permitted"
		...
		if (!withProxy) {
			...
			it("adjustVessel(): borrowing at non-zero base records the (drawn debt + fee) on the Vessel struct", async () => {
				...
				const emittedFee_Asset = toBN(th.getTRIFeeFromTRIBorrowingEvent(adjustmentTx_Asset))
				assert.isTrue(emittedFee_Asset.gt(toBN("0")))

				const D_newDebt_Asset = (await vesselManager.Vessels(D, erc20.address))[th.VESSEL_DEBT_INDEX]

				// Check debt on Vessel struct equals initial debt plus drawn debt plus emitted fee
		...
		it("adjustVessel(): borrowing at non-zero base rate sends requested amount to the user", async () => {
			...
			const D_TRIBalanceBefore = await debtToken.balanceOf(D)
			...
			await borrowerOperations.adjustVessel(erc20.address, 0, 0, TRIRequest_D, true, D, D, {
				from: D,
			})
		...
		it("adjustVessel(): borrowing at zero base rate sends total requested TRI to the user", async () => {
			...
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(40000, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: A },
			})
			await openVessel({
			...
			// D adjusts vessel
			const TRIRequest_D = toBN(dec(40, 18))
			const TRIBalanceAfter = await debtToken.balanceOf(D)

			await borrowerOperations.adjustVessel(erc20.address, 0, 0, TRIRequest_D, true, D, D, {
				from: D,
			})
			const TRIBalanceAfter_Asset = await debtToken.balanceOf(D)
			assert.isTrue(TRIBalanceAfter_Asset.eq(TRIBalanceAfter.add(TRIRequest_D)))
		})
		...
		it("adjustVessel(): reverts when calling address has no active vessel", async () => {
			...
			// Alice coll and debt increase(+1 ETH, +50TRI)
		...
		it("adjustVessel(): A vessel with ICR < CCR in Recovery Mode can adjust their vessel to ICR > CCR But collateral is blocked from minting, then unblock it", async () => {
			...
			await debtToken.emergencyStopMinting(erc20.address, true)
			...
			await debtToken.emergencyStopMinting(erc20.address, false)
		...
		it("adjustVessel(): debt increase in Recovery Mode charges no fee", async () => {
			...
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(200000, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: bob },
			})
			...
			// Check event is not emited
			try {
				await th.getEventArgByName(txAlice_Asset, "BorrowingFeePaid", "_feeAmount")
			} catch (e) {
				assert.isTrue(e === "The transaction logs do not contain event BorrowingFeePaid and arg _feeAmount")
			}
		...
		it("adjustVessel(): reverts when change would cause the ICR of the vessel to fall below the MCR", async () => {
			...
			// Bob attempts to increase debt by 100 TRI and 1 ether, i.e. a change that constitutes a 100% ratio of coll:debt.
		...
		it("adjustVessel(): With 0 coll change, doesnt change borrower's coll or ActivePool coll", async () => {
			...
			// Alice adjusts vessel. No coll change, and a debt increase (+50TRI)
		...
		it("adjustVessel(): updates borrower's debt and coll with an increase in both", async () => {
			...
			// Alice adjusts vessel. Coll and debt increase(+1 ETH, +50TRI)
			await borrowerOperations.adjustVessel(
				erc20.address,
				dec(1, "ether"),
				0,
		...
		it("adjustVessel(): updates borrower's debt and coll with a decrease in both", async () => {
			...
			// Alice adjusts vessel coll and debt decrease (-0.5 ETH, -50TRI)
		...
		it("adjustVessel(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
			...
			// Alice adjusts vessel - coll increase and debt decrease (+0.5 ETH, -50TRI)
		...
		it("adjustVessel(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
			...
			// Alice adjusts vessel - coll decrease and debt increase (0.1 ETH, 10TRI)
			await borrowerOperations.adjustVessel(
				erc20.address,
				0,
				dec(1, 17),
		...
		it("adjustVessel(): updates borrower's stake and totalStakes with a coll increase", async () => {
			...
			// Alice adjusts vessel - coll and debt increase (+1 ETH, +50 TRI)
		...
		it("adjustVessel(): Changes the TRI debt in ActivePool by requested decrease", async () => {
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(10, 18)),
			...
			const activePooL_TRIDebt_Before_Asset = await activePool.getDebtTokenBalance(erc20.address)
			assert.isTrue(activePooL_TRIDebt_Before_Asset.gt(toBN("0")))
			...
			const activePooL_TRIDebt_After_Asset = await activePool.getDebtTokenBalance(erc20.address)
			assert.isTrue(activePooL_TRIDebt_After_Asset.eq(activePooL_TRIDebt_Before_Asset.sub(toBN(dec(30, 18)))))
		})

		it("adjustVessel(): Changes the TRI debt in ActivePool by requested increase", async () => {
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(10000, 18)),
				ICR: toBN(dec(10, 18)),
			...
			const activePooL_TRIDebt_After_Asset = await activePool.getDebtTokenBalance(erc20.address)
			th.assertIsApproximatelyEqual(
				activePooL_TRIDebt_After_Asset,
				activePooL_TRIDebt_Before_Asset.add(toBN(dec(100, 18)))
			)
		})
		...
		it("adjustVessel(): Reverts if borrower has insufficient debtToken balance to cover his debt repayment", async () => {
			...
			//Confirm B's balance is less than 50 TRI

			const B_TRIBal_Asset = await debtToken.balanceOf(B)
			assert.isTrue(B_TRIBal_Asset.lt(bobDebt_Asset))

			const repayTRIPromise_B_Asset = borrowerOperations.adjustVessel(
				erc20.address,
				0,
				0,
				bobDebt_Asset,
			...
			await assertRevert(repayTRIPromise_B_Asset, "revert")
		})
		...
		it("closeVessel(): reverts when it would lower the TCR below CCR", async () => {
			...
			await openVessel({
				asset: erc20.address,
				ICR: toBN(dec(120, 16)),
				extraTRIAmount: toBN(dec(300, 18)),
				extraParams: { from: bob },
			})
		...
		it("closeVessel(): reverts when system is in Recovery Mode", async () => {
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(100000, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: alice },
			})
			await openVessel({
			...
			// Alice transfers her TRI to Bob and Carol so they can cover fees
		...
		it("closeVessel(): reverts when vessel is the only one in the system", async () => {
			...
			// Check she has more TRI than her vessel debt
			const aliceBal = await debtToken.balanceOf(alice)
		...
		it("closeVessel(): reduces a Vessel's collateral to zero", async () => {
			...
			const dennisTRI = await debtToken.balanceOf(dennis)

			assert.isTrue(dennisTRI.gt(toBN("0")))

			const aliceCollBefore_Asset = await getVesselEntireColl(alice, erc20.address)
			assert.isTrue(aliceCollBefore_Asset.gt(toBN("0")))

			// To compensate borrowing fees
			await debtToken.transfer(alice, dennisTRI.div(toBN(2)), { from: dennis })
		...
		it("closeVessel(): zero's the vessels reward snapshots", async () => {
			...
			const L_TRIDebt_A_Snapshot_Asset = (await vesselManager.rewardSnapshots(alice, erc20.address))[1]
			assert.isTrue(L_ETH_A_Snapshot_Asset.gt(toBN("0")))
			assert.isTrue(L_TRIDebt_A_Snapshot_Asset.gt(toBN("0")))
			...
			const L_TRIDebt_Snapshot_A_afterAliceCloses_Asset = (
				await vesselManager.rewardSnapshots(alice, erc20.address)
			)[1]

			assert.equal(L_ETH_Snapshot_A_afterAliceCloses_Asset, "0")
			assert.equal(L_TRIDebt_Snapshot_A_afterAliceCloses_Asset, "0")
		})
		...
		it("closeVessel(): reduces ActivePool debt by correct amount", async () => {
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(10_000, 18)),
				ICR: toBN(dec(2, 18)),
				extraParams: { from: dennis },
			})
			await openVessel({
		...
		it("closeVessel(): subtracts the debt of the closed Vessel from the Borrower's debtToken balance", async () => {
			...
			const alice_TRIBalance_Before = await debtToken.balanceOf(alice)
			assert.isTrue(alice_TRIBalance_Before.gt(toBN("0")))

			// close vessel
			await borrowerOperations.closeVessel(erc20.address, { from: alice })

			// check alice balance after
			const alice_TRIBalance_After = await debtToken.balanceOf(alice)
			// accept 0,5% error margin, as part of the fee will be refunded
			const error = Number(aliceDebt_Asset.mul(toBN(5)).div(toBN(1_000)))
			const debitated = alice_TRIBalance_Before.sub(alice_TRIBalance_After)
		...
		it("closeVessel(): applies pending rewards", async () => {
			...
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(1000000, 18)),
				ICR: toBN(dec(10, 18)),
				extraParams: { from: whale },
			})
			...
			const defaultPooL_TRIDebt_Asset = await defaultPool.getDebtTokenBalance(erc20.address)

			// Carol's liquidated coll (1 ETH) and drawn debt should have entered the Default Pool

			assert.isAtMost(th.getDifference(defaultPool_ETH_Asset, liquidatedColl_C_Asset), 100)
			assert.isAtMost(th.getDifference(defaultPooL_TRIDebt_Asset, liquidatedDebt_C_Asset), 100)
			...
			const defaultPooL_TRIDebt_afterAliceCloses_Asset = await defaultPool.getDebtTokenBalance(erc20.address)
			...
			assert.isAtMost(
				th.getDifference(
					defaultPooL_TRIDebt_afterAliceCloses_Asset,
					defaultPooL_TRIDebt_Asset.sub(pendingDebtReward_A_Asset)
				),
				1000
			)
			...
			const defaultPooL_TRIDebt_afterBobCloses_Asset = await defaultPool.getDebtTokenBalance(erc20.address)

			assert.isAtMost(th.getDifference(defaultPool_ETH_afterBobCloses_Asset, 0), 100000)
			assert.isAtMost(th.getDifference(defaultPooL_TRIDebt_afterBobCloses_Asset, 0), 100000)
		})

		it("closeVessel(): reverts if borrower has insufficient TRI balance to repay his entire debt", async () => {
			await openVessel({
				asset: erc20.address,
				extraTRIAmount: toBN(dec(15000, 18)),
				ICR: toBN(dec(2, 18)),
			...
			//Confirm Bob's TRI balance is less than his vessel debt

			const B_TRIBal_Asset = await debtToken.balanceOf(B)
			const B_vesselDebt_Asset = await getVesselEntireDebt(B, erc20.address)

			assert.isTrue(B_TRIBal_Asset.lt(B_vesselDebt_Asset))

			const closeVesselPromise_B_Asset = borrowerOperations.closeVessel(erc20.address, {
				from: B,
			})
			await assertRevert(closeVesselPromise_B_Asset, "BorrowerOps: Caller doesnt have enough TRI to make repayment")
		})
		...
		if (!withProxy) {
			...
			it("openVessel(): emits a VesselUpdated event with the correct collateral and debt", async () => {
				...
				const txC_Asset = (
					await openVessel({
						asset: erc20.address,
						extraTRIAmount: toBN(dec(3000, 18)),
						ICR: toBN(dec(2, 18)),
						extraParams: { from: C },
					})
				).tx
		...
		if (!withProxy) {
			...
			it("openVessel(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Vessel struct", async () => {
				...
				const D_TRIRequest = toBN(dec(20000, 18))
				...
				const emittedFee_Asset = toBN(th.getTRIFeeFromTRIBorrowingEvent(openVesselTx_Asset))
		...
		it("openVessel(): borrowing at non-zero base rate sends requested amount to the user", async () => {
			...
			assert.isTrue(TRIRequest_D.eq(TRIBalance_D))
		})
		...
		/* it("openVessel(): Borrowing at zero base rate charges minimum fee", async () => {
			...
			const TRIRequest = toBN(dec(10000, 18))
			const _TRIFee = toBN(th.getEventArgByName(txC, "TRIBorrowingFeePaid", "_TRIFee"))
			const _TRIFee_Asset = toBN(
				th.getEventArgByName(txC_Asset, "TRIBorrowingFeePaid", "_TRIFee")
			)
			const expectedFee = BORROWING_FEE_FLOOR.mul(toBN(TRIRequest)).div(toBN(dec(1, 18)))
			const expectedFee_Asset = BORROWING_FEE_FLOOR_ERC20.mul(toBN(TRIRequest)).div(
				toBN(dec(1, 18))
			)
			assert.isTrue(_TRIFee.eq(expectedFee))
			assert.isTrue(_TRIFee_Asset.eq(expectedFee_Asset))
		}) */
		...
		it("openVessel(): Can open a vessel with ICR >= CCR when system is in Recovery Mode", async () => {
			...
			// price drops to 1ETH:100TRI, reducing TCR below 150%
			await priceFeed.setPrice(erc20.address, "100000000000000000000")
			const price = await priceFeed.getPrice(erc20.address)
		...
		it("openVessel(): creates a new Vessel and assigns the correct collateral and debt amount", async () => {
			...
			const TRIRequestERC20 = MIN_NET_DEBT_ERC20
			await borrowerOperations.openVessel(erc20.address, dec(100, "ether"), MIN_NET_DEBT_ERC20, carol, carol, {
				from: alice,
			})

			// Get the expected debt based on the TRI request (adding fee and liq. reserve on top)
		...
		it("openVessel(): records up-to-date initial snapshots of L_ETH and L_TRIDebt", async () => {
			// --- SETUP ---
       ...
       and L_TRI should equal 18 TRI per-ether-staked. */

			const L_Asset = await vesselManager.L_Colls(erc20.address)
			const L_TRI_Asset = await vesselManager.L_Debts(erc20.address)

			assert.isTrue(L_Asset.gt(toBN("0")))
			assert.isTrue(L_TRI_Asset.gt(toBN("0")))
			...
			// Check Bob's snapshots of L_ETH and L_TRI equal the respective current values
			...
			const bob_TRIDebtRewardSnapshot_Asset = bob_rewardSnapshot_Asset[1]

			assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot_Asset, L_Asset), 1000)
			assert.isAtMost(th.getDifference(bob_TRIDebtRewardSnapshot_Asset, L_TRI_Asset), 1000)
		})
		...
		it("openVessel(): increases the Vessel's TRI debt by the correct amount", async () => {
			// check before
			...
			await borrowerOperations.openVessel(
				erc20.address,
				dec(100, "ether"),
				await getOpenVesselTRIAmount(dec(10000, 18), erc20.address),
				alice,
				alice,
				{ from: alice }
			)
		...
		it("openVessel(): increases TRI debt in ActivePool by the debt of the vessel", async () => {
			const activePooL_TRIDebt_Before_Asset = await activePool.getDebtTokenBalance(erc20.address)

			assert.equal(activePooL_TRIDebt_Before_Asset, 0)
			...
			assert.isTrue(activePooL_TRIDebt_After_Asset.eq(aliceDebt_Asset))
		})
		...
		describe("getNewTCRFromVesselChange() returns the correct TCR", async () => {
			...
			it("collChange = 0, debtChange = 0", async () => {
				...
				const vesselTRIAmount_Asset = await getOpenVesselTRIAmount(vesselTotalDebt, erc20.address)

				await borrowerOperations.openVessel(erc20.address, vesselColl, vesselTRIAmount_Asset, alice, alice, {
					from: alice,
				})
		...
		/* if (!withProxy) {
			it("closeVessel(): fails if owner cannot receive ETH", async () => {
				...
				// Alice sends TRI to NonPayable so its TRI balance covers its debt
...
/* TODO:
 ...
 changes with addColl, withdrawColl, withdrawTRI, withdrawTRI, etc. Can split them up and put them with
