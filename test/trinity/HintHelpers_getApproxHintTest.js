...
contract("VesselManagerOperations-HintHelpers", async accounts => {
	...
	/* Open a Vessel for each account. TRI debt is 200 TRI each, with collateral beginning at
  1.5 ether, and rising by 0.01 ether per Vessel.  Hence, the ICR of account (i + 1) is always 1% greater than the ICR of account i.
 */

	// Open Vessels in parallel, then withdraw TRI in parallel
	const makeVesselsInParallel = async (accounts, n) => {
		...
		const withdrawTRIpromises = activeAccounts.map(account => withdrawTRIfromVessel(account))
		await Promise.all(withdrawTRIpromises)
