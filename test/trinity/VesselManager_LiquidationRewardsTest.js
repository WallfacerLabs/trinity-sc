...
contract("VesselManager - Redistribution reward calculations", async accounts => {
    ...
    */
	...
	it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
		...
		const bob_TRIDebt_Asset = (await vesselManager.Vessels(bob, erc20.address))[th.VESSEL_DEBT_INDEX]
	...
	it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
		...
		/* Bob rewards:
     L1: 1/2*0.995 ETH, 55 TRI
     L2: (2.4975/3.995)*0.995 = 0.622 ETH , 110*(2.4975/3.995)= 68.77 TRIDebt
    coll: 3.1195 ETH
    debt: 233.77 TRIDebt
     Alice rewards:
    L1 1/2*0.995 ETH, 55 TRI
    L2 (1.4975/3.995)*0.995 = 0.3730 ETH, 110*(1.4975/3.995) = 41.23 TRIDebt
    coll: 1.8705 ETH
    debt: 146.23 TRIDebt
    totalColl: 4.99 ETH
    totalDebt 380 TRI
    */
		...
		const alice_TRIDebt_Asset = (await vesselManager.Vessels(alice, erc20.address))[th.VESSEL_DEBT_INDEX]
    ...
    */
	...
	it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
		...
		// Expect Bob now holds all Ether and TRIDebt in the system: 2.5 Ether and 300 TRI
	...
	it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
		...
		/* Bob rewards:
     L1: 0.4975 ETH, 55 TRI
     L2: (0.9975/2.495)*0.995 = 0.3978 ETH , 110*(0.9975/2.495)= 43.98 TRIDebt
    coll: (1 + 0.4975 - 0.5 + 0.3968) = 1.3953 ETH
    debt: (110 + 55 + 43.98 = 208.98 TRIDebt
     Alice rewards:
    L1 0.4975, 55 TRI
    L2 (1.4975/2.495)*0.995 = 0.5972 ETH, 110*(1.4975/2.495) = 66.022 TRIDebt
