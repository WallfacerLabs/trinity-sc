...
contract("VesselManager - in Recovery Mode", async accounts => {
	...
	it("liquidate(), with ICR < 100%: only redistributes to active Vessels - no offset to Stability Pool", async () => {
		...
		// --- TEST ---
		// price drops to 1ETH:100TRI, reducing TCR below 150%, and all Vessels below 100% ICR
		await priceFeed.setPrice(erc20.address, "100000000000000000000")
	...
	it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
		// --- SETUP ---
		//  Bob withdraws up to 2000 TRI of debt, bringing his ICR to 210%
		...
		// --- TEST ---
		// price drops to 1ETH:100TRI, reducing TCR to 117%
		await priceFeed.setPrice(erc20.address, "100000000000000000000")
	...
	it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
		...
		//  Bob withdraws up to 20000 TRI of debt, bringing his ICR to 210%
	...
	it("liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt", async () => {
		...
		// Alice deposits 390TRI to the Stability Pool
		await stabilityPool.provideToSP(spDeposit, validCollateral, { from: alice })
		...
		// check pool TRI before liquidation

		const stabilityPoolTRI_Before_Asset = (await stabilityPool.getTotalDebtTokenDeposits()).toString()
		assert.equal(stabilityPoolTRI_Before_Asset, "390000000000000000000")

		// check Pool reward term before liquidation

		assert.equal((await stabilityPool.P()).toString(), "1000000000000000000")

		/* Now, liquidate Bob. Liquidated coll is 21 ether, and liquidated debt is 2000 TRI.

    With 390 TRI in the StabilityPool, 390 TRI should be offset with the pool, leaving 0 in the pool.

    Stability Pool rewards for alice should be:
    TRILoss: 390TRI
    AssetGain: (390 / 2000) * 21*0.995 = 4.074525 ether
    After offsetting 390 TRI and 4.074525 ether, the remainders - 1610 TRI and 16.820475 ether - should be redistributed to all active Vessels.
   */
		// Liquidate Bob
		await vesselManagerOperations.liquidate(erc20.address, bob, { from: owner })
		...
		/* Now, check redistribution to active Vessels. Remainders of 1610 TRI and 16.82 ether are distributed.
    ...
    L_TRIDebt = 1610 / 6 = 268.333 TRI
    L_ETH = 16.820475 /6 =  2.8034125 ether
    */

		const L_TRIDebt_Asset = (await vesselManager.L_Debts(erc20.address)).toString()
	...
	it("liquidate(), with ICR > 110%, vessel has lowest ICR, and StabilityPool is empty: does nothing", async () => {
		...
		assert.equal(L_TRIDebt_Asset, "0")
	...
	it("liquidate(), with 110% < ICR < TCR, and StabilityPool TRI > debt to liquidate: offsets the vessel entirely with the pool", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 TRI of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 TRI of debt, resulting in ICR of 240%. Bob has lowest ICR.

		const { collateral: B_coll_Asset, totalDebt: B_totalDebt_Asset } = await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(240, 16)),
			extraTRIAmount: dec(250, 18),
			extraParams: { from: bob },
		})
		await openVessel({
			asset: erc20.address,
		...
		// Alice deposits TRI in the Stability Pool
    ...
    */
		...
		const aliceExpectedETHGain_Asset = await getDepositorGain(alice, validCollateral, erc20.address)
	...
	it("liquidate(), with ICR% = 110 < TCR, and StabilityPool TRI > debt to liquidate: offsets the vessel entirely with the pool, there’s no collateral surplus", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 TRI of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 TRI of debt, resulting in ICR of 220%. Bob has lowest ICR.
    ...
    */
	...
	it("liquidate(), with 110% < ICR < TCR, and StabilityPool TRI > debt to liquidate: removes stake and updates totalStakes", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 TRI of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 TRI of debt, resulting in ICR of 240%. Bob has lowest ICR.
	...
	it("liquidate(), with 110% < ICR < TCR, and StabilityPool TRI > debt to liquidate: updates system snapshots", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 TRI of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 TRI of debt, resulting in ICR of 240%. Bob has lowest ICR.
	...
	it("liquidate(), with 110% < ICR < TCR, and StabilityPool TRI > debt to liquidate: closes the Vessel", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 TRI of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 TRI of debt, resulting in ICR of 240%. Bob has lowest ICR.
	...
	it("liquidate(), with 110% < ICR < TCR, and StabilityPool TRI > debt to liquidate: can liquidate vessels out of order", async () => {
		// taking out 1000 TRI, CR of 200%
		...
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
			extraTRIAmount: totalLiquidatedDebt_Asset,
  ...
  TRI is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

	it("liquidate(), with ICR > 110%, and StabilityPool TRI < liquidated debt: Vessel remains active", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 TRI of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 TRI of debt, resulting in ICR of 240%. Bob has lowest ICR.
		...
		// Alice deposits 1490 TRI in the Stability Pool
		...
		/* Since the pool only contains 100 TRI, and Bob's pre-liquidation debt was 250 TRI,
	...
	it("liquidate(), with ICR > 110%, and StabilityPool TRI < liquidated debt: Vessel remains in VesselOwners array", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 TRI of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 TRI of debt, resulting in ICR of 240%. Bob has lowest ICR.
		...
		// Alice deposits 100 TRI in the Stability Pool
	...
	it("liquidate(), with ICR > 110%, and StabilityPool TRI < liquidated debt: nothing happens", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 TRI of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 TRI of debt, resulting in ICR of 240%. Bob has lowest ICR.
		...
		/*  Since Bob's debt (250 TRI) is larger than all TRI in the Stability Pool, Liquidation won’t happen
    ...
    */
	...
	it("liquidate(), with ICR > 110%, and StabilityPool TRI < liquidated debt: updates system shapshots", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 TRI of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 TRI of debt, resulting in ICR of 240%. Bob has lowest ICR.
	...
	it("liquidate(), with ICR > 110%, and StabilityPool TRI < liquidated debt: causes correct Pool offset and ETH gain, and doesn't redistribute to active vessels", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 TRI of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 TRI of debt, resulting in ICR of 240%. Bob has lowest ICR.
    ...
    Redistribution rewards-per-unit-staked should be zero. */
		...
		const L_TRIDebt_After_Asset = (await vesselManager.L_Debts(erc20.address)).toString()
		const L_ETH_After_Asset = (await vesselManager.L_Colls(erc20.address)).toString()

		assert.equal(L_TRIDebt_After_Asset, "0")
		assert.equal(L_ETH_After_Asset, "0")
	})

	it("liquidate(), with ICR > 110%, and StabilityPool TRI < liquidated debt: ICR of non liquidated vessel does not change", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 TRI of debt, and Dennis up to 150, resulting in ICRs of 266%.
		// Bob withdraws up to 250 TRI of debt, resulting in ICR of 240%. Bob has lowest ICR.
		// Carol withdraws up to debt of 240 TRI, -> ICR of 250%.
		...
		//Check SP TRI has been completely emptied
		...
		// Alice provides another 50 TRI to pool
	...
	it("liquidate() with ICR > 110%, and StabilityPool TRI < liquidated debt: total liquidated coll and debt is correct", async () => {
		// Whale provides 50 TRI to the SP
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(300, 16)),
			extraTRIAmount: dec(50, 18),
			extraParams: { from: whale },
		})
	...
	it("liquidate(): Doesn't liquidate undercollateralized vessel if it is the only vessel in the system", async () => {
		// Alice creates a single vessel with 0.62 ETH and a debt of 62 TRI, and provides 10 TRI to SP
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
			extraParams: { from: alice },
	...
	it("liquidate(): does nothing if vessel has >= 110% ICR and the Stability Pool is empty", async () => {
		...
		const TRIinSP_Asset = (await stabilityPool.getTotalDebtTokenDeposits()).toString()
		assert.equal(TRIinSP_Asset, "0")
	...
	it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
		...
		// Defaulter opens with 60 TRI, 0.6 ETH
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
    ...
    Therefore Alice and Bob above the MCR, Carol is below */
		...
		// Liquidate defaulter. 30 TRI and 0.3 ETH is distributed uniformly between A, B and C. Each receive 10 TRI, 0.1 ETH
    ...
    check that Bob's raw coll and debt has not changed, and that his "raw" ICR is above the MCR */
		...
		/*  Since there is 0 TRI in the stability Pool, A, with ICR >110%, should stay active.
	...
	it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no vessel", async () => {
		...
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(220, 16)),
			extraTRIAmount: spDeposit_Asset,
			extraParams: { from: bob },
		})
		...
		const dennis_ETHGain_After_Asset = (await getDepositorGain(dennis, validCollateral, erc20.address)).toString()
	...
	it("liquidate(): does not alter the liquidated user's token balance", async () => {
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(220, 16)),
			extraTRIAmount: dec(1000, 18),
			extraParams: { from: whale },
		})

		const { TRIAmount: A_TRIAmount_Asset } = await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
			extraTRIAmount: dec(300, 18),
			extraParams: { from: alice },
		})
		const { TRIAmount: B_TRIAmount_Asset } = await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
			extraTRIAmount: dec(200, 18),
			extraParams: { from: bob },
		})
		const { TRIAmount: C_TRIAmount_Asset } = await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(206, 16)),
		...
		assert.equal((await debtToken.balanceOf(alice)).toString(), A_TRIAmount_Asset)
		assert.equal((await debtToken.balanceOf(bob)).toString(), B_TRIAmount_Asset)

		assert.equal((await debtToken.balanceOf(carol)).toString(), C_TRIAmount_Asset)
	...
	it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
		// --- SETUP ---
		// Alice withdraws up to 1500 TRI of debt, resulting in ICRs of 266%.
		// Bob withdraws up to 480 TRI of debt, resulting in ICR of 240%. Bob has lowest ICR.

		const { collateral: B_coll_Asset, totalDebt: B_totalDebt_Asset } = await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(240, 16)),
			extraTRIAmount: dec(480, 18),
			extraParams: { from: bob },
		})
		await openVessel({
			asset: erc20.address,
		...
		// Bob re-opens the vessel, price 200, total debt 80 TRI, ICR = 120% (lowest one)
		...
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(266, 16)),
			extraTRIAmount: B_netDebt_2_Asset,
			extraParams: { from: dennis },
		})
	...
	it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
		// --- SETUP ---
		// Bob withdraws up to 90 TRI of debt, resulting in ICR of 222%
		// Dennis withdraws to 150 TRI of debt, resulting in ICRs of 266%.
		...
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(266, 16)),
			extraTRIAmount: B_netDebt_Asset,
			extraParams: { from: dennis },
		})
		...
		// Bob re-opens the vessel, price 200, total debt 250 TRI, ICR = 240% (lowest one)
		...
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(266, 16)),
			extraTRIAmount: B_totalDebt_2_Asset,
			extraParams: { from: alice },
		})
	...
	it("liquidateVessels(): With all ICRs > 110%, Liquidates Vessels until system leaves recovery mode", async () => {
		...
		// Everyone withdraws some TRI from their Vessel, resulting in different ICRs
		...
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(400, 16)),
			extraTRIAmount: liquidationAmount_Asset,
			extraParams: { from: alice },
		})

		// Alice deposits TRI to Stability Pool
		await stabilityPool.provideToSP(liquidationAmount_Asset, validCollateral, { from: alice })

		// price drops
		// price drops to 1ETH:90TRI, reducing TCR below 150%
    ...
    */
	...
	it("liquidateVessels(): Liquidates Vessels until 1) system has left recovery mode AND 2) it reaches a Vessel with ICR >= 110%", async () => {
		...
		// price drops to 1ETH:85TRI, reducing TCR below 150%
   ...
   (see calculations in Google Sheet)
	...
	it("liquidateVessels(): closes every Vessel with ICR < MCR, when n > number of undercollateralized vessels", async () => {
		...
		// Price drops to 1ETH:100TRI, reducing Bob and Carol's ICR below MCR
		await priceFeed.setPrice(erc20.address, dec(100, 18))
		const price = await priceFeed.getPrice(erc20.address)
	...
	it("liquidateVessels(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
		...
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(198, 16)),
			extraTRIAmount: dec(101, 18),
			extraParams: { from: defaulter_1 },
		})
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(184, 16)),
			extraTRIAmount: dec(217, 18),
			extraParams: { from: defaulter_2 },
		})
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(183, 16)),
			extraTRIAmount: dec(328, 18),
			extraParams: { from: defaulter_3 },
		})
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(186, 16)),
			extraTRIAmount: dec(431, 18),
		...
		// Check Stability Pool has 500 TRI
   ...
   (because his pending rewards bring his ICR < MCR) */
	...
	it("liquidateVessels(): does nothing if all vessels have ICR > 110% and Stability Pool is empty", async () => {
		...
		// Confirm 0 TRI in Stability Pool
		assert.equal((await stabilityPool.getTotalDebtTokenDeposits()).toString(), "0")
	...
	it("liquidateVessels(): does not affect the liquidated user's token balances", async () => {
		...
		const { TRIAmount: TRIAmountD_Asset } = await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
			extraParams: { from: dennis },
		})
		const { TRIAmount: TRIAmountE_Asset } = await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(133, 16)),
			extraParams: { from: erin },
		})
		const { TRIAmount: TRIAmountF_Asset } = await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(111, 16)),
			extraParams: { from: freddy },
		})
		...
		assert.equal((await debtToken.balanceOf(dennis)).toString(), TRIAmountD_Asset)
		assert.equal((await debtToken.balanceOf(erin)).toString(), TRIAmountE_Asset)
		assert.equal((await debtToken.balanceOf(freddy)).toString(), TRIAmountF_Asset)
	...
	it("liquidateVessels(): Liquidating vessels at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
		// Whale provides TRI to the SP
		const { TRIAmount: W_TRIAmount_Asset } = await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(300, 16)),
			extraTRIAmount: dec(4000, 18),
			extraParams: { from: whale },
		})

		await stabilityPool.provideToSP(W_TRIAmount_Asset, validCollateral, { from: whale })

		const {
			TRIAmount: A_TRIAmount_Asset,
		...
		await stabilityPool.provideToSP(A_TRIAmount_Asset, validCollateral, { from: alice })
		await stabilityPool.provideToSP(B_TRIAmount_Asset, validCollateral, { from: bob })

		const totalDeposit_Asset = W_TRIAmount_Asset.add(A_TRIAmount_Asset).add(B_TRIAmount_Asset)
    ...
    Bob:   240 TRI
    Carol: 0 TRI
    Total TRI in Pool: 680 TRI
    Then, liquidation hits A,B,C:
    Total liquidated debt = 100 + 300 + 100 = 500 TRI
    Total liquidated ETH = 1 + 3 + 1 = 5 ETH
    Whale TRI Loss: 500 * (400/680) = 294.12 TRI
    Alice TRI Loss:  500 *(40/680) = 29.41 TRI
    Bob TRI Loss: 500 * (240/680) = 176.47 TRI
    Whale remaining deposit: (400 - 294.12) = 105.88 TRI
    Alice remaining deposit: (40 - 29.41) = 10.59 TRI
    Bob remaining deposit: (240 - 176.47) = 63.53 TRI
    Whale ETH Gain: 5*0.995 * (400/680) = 2.93 ETH
    Alice ETH Gain: 5*0.995 *(40/680) = 0.293 ETH
    Bob ETH Gain: 5*0.995 * (240/680) = 1.76 ETH
    Total remaining deposits: 180 TRI
    Total ETH gain: 5*0.995 ETH */

		// Check remaining TRI Deposits and ETH gain, for whale and depositors whose vessels were liquidated
		...
		const whale_ETHGain_Asset = await getDepositorGain(whale, validCollateral, erc20.address)
		const alice_ETHGain_Asset = await getDepositorGain(alice, validCollateral, erc20.address)
		const bob_ETHGain_Asset = await getDepositorGain(bob, validCollateral, erc20.address)
		...
		assert.isAtMost(
			th.getDifference(
				...
				W_TRIAmount_Asset.sub(liquidatedDebt_Asset.mul(W_TRIAmount_Asset).div(totalDeposit_Asset))
			),
			100000
		)
		assert.isAtMost(
			th.getDifference(
				...
				A_TRIAmount_Asset.sub(liquidatedDebt_Asset.mul(A_TRIAmount_Asset).div(totalDeposit_Asset))
			),
			100000
		)
		assert.isAtMost(
			th.getDifference(
				...
				B_TRIAmount_Asset.sub(liquidatedDebt_Asset.mul(B_TRIAmount_Asset).div(totalDeposit_Asset))
			),
			100000
		)
		...
		const total_TRIinSP_Asset = (await stabilityPool.getTotalDebtTokenDeposits()).toString()
	...
	it("liquidateVessels(): Liquidating vessels at ICR <=100% with SP deposits does not alter their deposit or ETH gain", async () => {
		// Whale provides 400 TRI to the SP
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(300, 16)),
		...
		// Check TRI and ETH in Pool  before
		const TRIinSP_Before_Asset = (await stabilityPool.getTotalDebtTokenDeposits()).toString()
		const ETHinSP_Before_Asset = (await stabilityPool.getCollateral(erc20.address)).toString()

		assert.equal(TRIinSP_Before_Asset, dec(800, 18))
		...
		// Check TRI and ETH in Pool after
		const TRIinSP_After_Asset = (await stabilityPool.getTotalDebtTokenDeposits()).toString()
		const ETHinSP_After_Asset = (await stabilityPool.getCollateral(erc20.address)).toString()

		assert.equal(TRIinSP_Before_Asset, TRIinSP_After_Asset)
		...
		const whale_ETHGain_After_Asset = await getDepositorGain(whale, validCollateral, erc20.address)
		const alice_ETHGain_After_Asset = await getDepositorGain(alice, validCollateral, erc20.address)
		const bob_ETHGain_After_Asset = await getDepositorGain(bob, validCollateral, erc20.address)
    ...
    That leaves 50 TRI in the Pool to absorb exactly half of Carol's debt (100) */
		await vesselManagerOperations.liquidateVessels(erc20.address, 10)
	...
	it("liquidateVessels() with a non fullfilled liquidation: still can liquidate further vessels after the non-liquidated, emptied pool", async () => {
		...
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
			extraTRIAmount: D_totalDebt_Asset,
			extraParams: { from: carol },
		})
		await openVessel({
			asset: erc20.address,
     ...
     That leaves 97 TRI in the Pool that won’t be enough to absorb Carol,
	...
	it("liquidateVessels() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
		...
		/* Liquidate vessels. Vessels are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated.
    That leaves 50 TRI in the Pool that won’t be enough to absorb any other vessel */
		await vesselManagerOperations.liquidateVessels(erc20.address, 10)

		// Expect system debt reduced by 203 TRI and system coll 2.3 ETH
		const entireSystemCollAfter_Asset = await vesselManager.getEntireSystemColl(erc20.address)
		const entireSystemDebtAfter_Asset = await vesselManager.getEntireSystemDebt(erc20.address)
    ...
    */
	...
	it("batchLiquidateVessels(): Liquidates all vessels with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
		...
		await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(426, 16)),
			extraTRIAmount: A_totalDebt_Asset,
			extraParams: { from: whale },
		})
    ...
    */
	...
	it("batchLiquidateVessels() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
		...
		// Expect system debt reduced by 203 TRI and system coll by 2 ETH
	...
	it("batchLiquidateVessels(), with 110% < ICR < TCR, and StabilityPool TRI > debt to liquidate: can liquidate vessels out of order", async () => {
		const { totalDebt: A_totalDebt_Asset } = await openVessel({
			asset: erc20.address,
			ICR: toBN(dec(200, 16)),
			extraParams: { from: alice },
		...
		// Whale provides 1000 TRI to the SP
	...
	it("batchLiquidateVessels(): skips liquidation of vessels with ICR > TCR, regardless of Stability Pool size", async () => {
		...
		// Whale withdraws entire deposit, and re-deposits 132 TRI
		...
		// Expected Stability Pool to fully absorb B (92 TRI + 10 virtual debt),
	...
	it("batchLiquidateVessels(): emits liquidation event with correct values when all vessels have ICR > 110% and Stability Pool covers a subset of vessels, including a partial", async () => {
		...
		// Whale opens vessel and adds 220 TRI to SP
