...
contract("VesselManager", async accounts => {
	...
	describe("Vessel Manager", async () => {
		...
		describe("Liquidations", async () => {
			it("liquidate(): closes a Vessel that has ICR < MCR", async () => {
				...
				// Alice increases debt to 180 TRI, lowering her ICR to 1.11
				...
				// price drops to 1ETH:100TRI, reducing Alice's ICR below MCR
			...
			it("liquidate(): decreases ActivePool ETH and TRIDebt by correct amounts", async () => {
				// --- SETUP ---
				...
				// check ActivePool ETH and TRI debt before
				...
				const activePooL_TRIDebt_Before_Asset = (await activePool.getDebtTokenBalance(erc20.address)).toString()
				...
				th.assertIsApproximatelyEqual(activePooL_TRIDebt_Before_Asset, A_totalDebt_Asset.add(B_totalDebt_Asset))

				// price drops to 1ETH:100TRI, reducing Bob's ICR below MCR
				await priceFeed.setPrice(erc20.address, "100000000000000000000")

				// Confirm system is not in Recovery Mode
				assert.isFalse(await th.checkRecoveryMode(contracts.core, erc20.address))

				/* close Bob's Vessel. Should liquidate his ether and TRI,
    leaving Alice’s ether and TRI debt in the ActivePool. */
				await vesselManagerOperations.liquidate(erc20.address, bob, { from: owner })

				// check ActivePool ETH and TRI debt
				...
				const activePooL_TRIDebt_After_Asset = (await activePool.getDebtTokenBalance(erc20.address)).toString()
				...
				th.assertIsApproximatelyEqual(activePooL_TRIDebt_After_Asset, A_totalDebt_Asset)
			})

			it("liquidate(): increases DefaultPool ETH and TRI debt by correct amounts", async () => {
				// --- SETUP ---
				await openVessel({
					asset: erc20.address,
					ICR: toBN(dec(4, 18)),
				...
				// check DefaultPool ETH and TRI debt before
				...
				const defaultPooL_TRIDebt_Before_Asset = (await defaultPool.getDebtTokenBalance(erc20.address)).toString()

				assert.equal(defaultPool_ETH_Before_Asset, "0")
				assert.equal(defaultPool_RawEther_Before_Asset, "0")
				assert.equal(defaultPooL_TRIDebt_Before_Asset, "0")
    ...
    Check all remaining vessels in the array are in the correct order */
			...
			it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
				...
				// close Bob's Vessel.  His ether*0.995 and TRI should be added to the DefaultPool.
				await vesselManagerOperations.liquidate(erc20.address, bob, { from: owner })
    ...
    from the liquidation of Bob's Vessel */
			...
			it("liquidate(): updates the L_ETH and L_TRIDebt reward-per-unit-staked totals", async () => {
				// --- SETUP ---
				const { collateral: A_collateral_Asset, totalDebt: A_totalDebt_Asset } = await openVessel({
					asset: erc20.address,
					ICR: toBN(dec(8, 18)),
				...
				// price drops to 1ETH:100TRI, reducing Carols's ICR below MCR
				await priceFeed.setPrice(erc20.address, "100000000000000000000")
				...
				// Carol's ether*0.995 and TRI should be added to the DefaultPool.
				...
				assert.isAtMost(th.getDifference(L_TRIDebt_AfterCarolLiquidated_Asset, L_TRIDebt_expected_1_Asset), 100)

				// Bob now withdraws TRI, bringing his ICR to 1.11
				const { increasedTotalDebt: B_increasedTotalDebt_Asset } = await withdrawTRI({
					asset: erc20.address,
					ICR: toBN(dec(111, 16)),
					extraParams: { from: bob },
				})
				...
				// price drops to 1ETH:50TRI, reducing Bob's ICR below MCR
				...
				const L_TRIDebt_AfterBobLiquidated_Asset = await vesselManager.L_Debts(erc20.address)
   ...
   L_TRIDebt = (180 / 20) + (890 / 10) = 98 TRI */
				...
				const L_TRIDebt_expected_2 = L_TRIDebt_expected_1_Asset.add(
					B_totalDebt_Asset.add(B_increasedTotalDebt_Asset)
						.add(B_collateral_Asset.mul(L_TRIDebt_expected_1_Asset).div(mv._1e18BN))
						.mul(mv._1e18BN)
						.div(A_collateral_Asset)
				)
				assert.isAtMost(th.getDifference(L_ETH_AfterBobLiquidated_Asset, L_ETH_expected_2_Asset), 100)
				assert.isAtMost(th.getDifference(L_TRIDebt_AfterBobLiquidated_Asset, L_TRIDebt_expected_2), 100)
			})
			...
			it("liquidate(): liquidates undercollateralized vessel if there are two vessels in the system", async () => {
				...
				// Alice creates a single vessel with 0.7 ETH and a debt of 70 TRI, and provides 10 TRI to SP
				await openVessel({
					asset: erc20.address,
					ICR: toBN(dec(2, 18)),
					extraParams: { from: alice },
			...
			it("liquidate(): given the same price and no other vessel changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening vessels", async () => {
				...
				await stabilityPool.provideToSP(spDeposit, validCollateral, { from: whale })
			...
			it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no vessel", async () => {
				...
				await openVessel({
					asset: erc20.address,
					ICR: toBN(dec(218, 16)),
					extraTRIAmount: toBN(dec(100, 18)),
					extraParams: { from: carol },
				})
				...
				//Dennis provides TRI to SP
				await stabilityPool.provideToSP(spDeposit, validCollateral, { from: dennis })
			...
			it("liquidate(): does not liquidate a SP depositor's vessel with ICR > 110%, and does not affect their SP deposit or ETH gain", async () => {
				...
				//Bob provides TRI to SP
				await stabilityPool.provideToSP(spDeposit, validCollateral, { from: bob })
			...
			it("liquidate(): liquidates a SP depositor's vessel with ICR < 110%, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
				...
				//Bob provides TRI to SP
				await stabilityPool.provideToSP(B_spDeposit, validCollateral, { from: bob })
				...
				/* Alice's TRI Loss = (300 / 400) * 200 = 150 TRI
       Alice's ETH gain = (300 / 400) * 2*0.995 = 1.4925 ETH

       Bob's TRILoss = (100 / 400) * 200 = 50 TRI
     ...
     Check Bob' SP deposit has been reduced to 50 TRI, and his ETH gain has increased to 1.5 ETH. */
			...
			it("liquidate(): does not alter the liquidated user's token balance", async () => {
				...
				const { TRIAmount: B_TRIAmount_Asset } = await openVessel({
					asset: erc20.address,
					ICR: toBN(dec(2, 18)),
					extraTRIAmount: toBN(dec(200, 18)),
					extraParams: { from: bob },
				})
				...
				assert.equal((await debtToken.balanceOf(alice)).toString(), A_TRIAmount_Asset.toString())
				assert.equal((await debtToken.balanceOf(bob)).toString(), B_TRIAmount_Asset.toString())
				assert.equal((await debtToken.balanceOf(carol)).toString(), C_TRIAmount_Asset.toString())
			})
    ...
    Therefore Alice and Bob above the MCR, Carol is below */
				...
				/* Liquidate defaulter. 30 TRI and 0.3 ETH is distributed between A, B and C.

    A receives (30 * 2/4) = 15 TRI, and (0.3*2/4) = 0.15 ETH
    B receives (30 * 1/4) = 7.5 TRI, and (0.3*1/4) = 0.075 ETH
    C receives (30 * 1/4) = 7.5 TRI, and (0.3*1/4) = 0.075 ETH
    */
				await vesselManagerOperations.liquidate(erc20.address, defaulter_1)
   ...
   (because his pending rewards bring his ICR < MCR) */
			...
			it("liquidateVessels(): liquidates a Vessel that a) was skipped in a previous liquidation and b) has pending rewards", async () => {
				...
				// A adds 10 TRI to the SP, but less than C's debt
				await stabilityPool.provideToSP(dec(10, 18), validCollateral, { from: A })
				...
				const pendingTRIDebt_C_Asset = await vesselManager.getPendingDebtTokenReward(erc20.address, C)
				const defaultPoolETH_Asset = await defaultPool.getAssetBalance(erc20.address)
				const defaultPoolTRIDebt_Asset = await defaultPool.getDebtTokenBalance(erc20.address)

				assert.isTrue(pendingETH_C_Asset.lte(defaultPoolETH_Asset))
				assert.isTrue(pendingTRIDebt_C_Asset.lte(defaultPoolTRIDebt_Asset))
				//Check only difference is dust

				assert.isAtMost(th.getDifference(pendingETH_C_Asset, defaultPoolETH_Asset), 1000)
				assert.isAtMost(th.getDifference(pendingTRIDebt_C_Asset, defaultPoolTRIDebt_Asset), 1000)

				// Confirm system is still in Recovery Mode
				assert.isTrue(await th.checkRecoveryMode(contracts.core, erc20.address))

				// D and E fill the Stability Pool, enough to completely absorb C's debt of 70
				await stabilityPool.provideToSP(dec(50, 18), validCollateral, { from: D })
				await stabilityPool.provideToSP(dec(50, 18), validCollateral, { from: E })
    ...
    Check Alice is above MCR, Bob below, Carol below. */
			...
			it("liquidateVessels(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
				...
				await openVessel({
					asset: erc20.address,
					ICR: toBN(dec(100, 18)),
					extraTRIAmount: toBN(dec(500, 18)),
					extraParams: { from: whale },
				})
			...
			it("liquidateVessels(): liquidating vessels with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
				...
				await openVessel({
					asset: erc20.address,
					ICR: toBN(dec(100, 18)),
					extraTRIAmount: whaleDeposit,
					extraParams: { from: whale },
				})
				await stabilityPool.provideToSP(whaleDeposit, validCollateral, { from: whale })
				...
				const { collateral: A_coll_Asset, totalDebt: A_debt_Asset } = await openVessel({
					asset: erc20.address,
					ICR: toBN(dec(2, 18)),
					extraTRIAmount: A_deposit,
					extraParams: { from: alice },
				})
				const { collateral: B_coll_Asset, totalDebt: B_debt_Asset } = await openVessel({
					asset: erc20.address,
					ICR: toBN(dec(2, 18)),
					extraTRIAmount: B_deposit,
					extraParams: { from: bob },
				})
				const { collateral: C_coll_Asset, totalDebt: C_debt_Asset } = await openVessel({
					asset: erc20.address,
				...
				await stabilityPool.provideToSP(A_deposit, validCollateral, { from: alice })
				await stabilityPool.provideToSP(B_deposit, validCollateral, { from: bob })
    ...
    Total TRI in Pool: 800 TRI

    Then, liquidation hits A,B,C:

    Total liquidated debt = 150 + 350 + 150 = 650 TRI
    Total liquidated ETH = 1.1 + 3.1 + 1.1 = 5.3 ETH

    whale TRI loss: 650 * (400/800) = 325 TRI
    alice TRI loss:  650 *(100/800) = 81.25 TRI
    bob TRI loss: 650 * (300/800) = 243.75 TRI

    whale remaining deposit: (400 - 325) = 75 TRI
    alice remaining deposit: (100 - 81.25) = 18.75 TRI
    bob remaining deposit: (300 - 243.75) = 56.25 TRI
    ...
    Total remaining deposits: 150 TRI
    Total ETH gain: 4.975 ETH */
		...
		describe("Batch Liquidations", async () => {
			...
			it("batchLiquidateVessels(): closes every vessel with ICR < MCR in the given array", async () => {
				...
				// Price drops to 1ETH:100TRI, reducing A, B, C ICR below MCR
				await priceFeed.setPrice(erc20.address, dec(100, 18))
				const price = await priceFeed.getPrice(erc20.address)
