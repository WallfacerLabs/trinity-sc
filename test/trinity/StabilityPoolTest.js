	...
	describe("Stability Pool Mechanisms", async () => {
		...
		describe("Withdrawing", async () => {
			...
			it("withdrawFromSP(): Subsequent deposit and withdrawal attempt from same account, with no intermediate liquidations, withdraws zero collateral", async () => {
				...
				// Alice makes deposit #1: 15000 TRI
				await openVessel({
					asset: erc20.address,
				...
				// Alice retrieves all of her entitled TRI:
			...
			it("withdrawFromSP(): it correctly updates the user's TRI and collateral snapshots of entitled reward per unit staked", async () => {
				// --- SETUP ---
				// Whale deposits 185000 TRI in StabilityPool
				await openWhaleVessel(erc20, (icr = 10), (extraDebtTokenAmt = 1_000_000))
				await stabilityPool.provideToSP(dec(185_000, 18), validCollateral, { from: whale })
				...
				// Alice retrieves part of her entitled TRI: 9000 TRI
			...
			it("withdrawFromSP(): decreases StabilityPool ERC20", async () => {
				...
				// Alice makes deposit #1: 15,000 TRI
				await openVessel({
					asset: erc20.address,
					extraTRIAmount: toBN(dec(15_000, 18)),
					ICR: toBN(dec(10, 18)),
					extraParams: { from: alice },
				})
				await stabilityPool.provideToSP(dec(15_000, 18), validCollateral, { from: alice })
			...
			it("withdrawFromSP(): withdraw from SP forfeiting gains", async () => {
				...
				// Alice makes deposit #1: 1,000 TRI
				await openVessel({
					asset: erc20.address,
					extraTRIAmount: toBN(dec(1000, 18)),
					ICR: toBN(dec(10, 18)),
					extraParams: { from: alice },
				})
			...
			it("withdrawFromSP(): increases depositor's TRI token balance by the expected amount", async () => {
				await openWhaleVessel(erc20, (icr = 10))
				...
				/* From an offset of 10000 TRI, each depositor receives
	  TRILoss = 1666.6666666666666666 TRI

	  and thus with a deposit of 10000 TRI, each should withdraw 8333.3333333333333333 TRI (in practice, slightly less due to rounding error)
	  */
				...
				// Expect Alice's debt token balance increase be very close to 8333.3333333333333333 TRI
				await stabilityPool.withdrawFromSP(dec(10000, 18), validCollateral, { from: alice })
				...
				// expect Bob's debt token balance increase to be very close to  13333.33333333333333333 TRI
				await stabilityPool.withdrawFromSP(dec(10000, 18), validCollateral, { from: bob })
			...
			it("withdrawFromSP(): Request to withdraw > caller's deposit only withdraws the caller's compounded deposit", async () => {
				...
				const alice_TRI_Balance_Before = await debtToken.balanceOf(alice)
				const bob_TRI_Balance_Before = await debtToken.balanceOf(bob)
				...
				const TRIinSP_Before = await stabilityPool.getTotalDebtTokenDeposits()
				...
				// Check Bob's TRI balance has risen by only the value of his compounded deposit
				const bob_expectedTRIBalance = bob_TRI_Balance_Before.add(bob_Deposit_Before).toString()
				const bob_TRI_Balance_After = (await debtToken.balanceOf(bob)).toString()
				assert.equal(bob_TRI_Balance_After, bob_expectedTRIBalance)

				// Alice attempts to withdraws 2309842309.000000000000000000 TRI from the Stability Pool
				await stabilityPool.withdrawFromSP("2309842309000000000000000000", validCollateral, { from: alice })

				// Check Alice's TRI balance has risen by only the value of her compounded deposit
				const alice_expectedTRIBalance = alice_TRI_Balance_Before.add(alice_Deposit_Before).toString()
				const alice_TRI_Balance_After = (await debtToken.balanceOf(alice)).toString()
				assert.equal(alice_TRI_Balance_After, alice_expectedTRIBalance)

				// Check TRI in Stability Pool has been reduced by only Alice's compounded deposit and Bob's compounded deposit
				const expectedTRIinSP = TRIinSP_Before.sub(alice_Deposit_Before).sub(bob_Deposit_Before).toString()
				const TRIinSP_After = (await stabilityPool.getTotalDebtTokenDeposits()).toString()
				assert.equal(TRIinSP_After, expectedTRIinSP)
			})

			it("withdrawFromSP(): Request to withdraw 2^256-1 TRI only withdraws the caller's compounded deposit", async () => {
				// --- SETUP ---
				await openWhaleVessel(erc20, (icr = 10))
				...
				// Bob attempts to withdraws maxBytes32 TRI from the Stability Pool
				...
				// Check TRI in Stability Pool has been reduced by only  Bob's compounded deposit
				const expectedTRIinSP = TRIinSP_Before.sub(bob_Deposit_Before).toString()
				const TRIinSP_After = (await stabilityPool.getTotalDebtTokenDeposits()).toString()
				assert.equal(TRIinSP_After, expectedTRIinSP)
			})
			...
			it("withdrawFromSP(): caller can withdraw full deposit and collateral gain during Recovery Mode", async () => {
				...
				// A, B, C provides 10000, 5000, 3000 TRI to SP
				await stabilityPool.provideToSP(dec(10000, 18), validCollateral, { from: alice })
				...
				const alice_TRI_Balance_Before = await debtToken.balanceOf(alice)
				const bob_TRI_Balance_Before = await debtToken.balanceOf(bob)
				const carol_TRI_Balance_Before = await debtToken.balanceOf(carol)
				...
				const TRIinSP_Before = await stabilityPool.getTotalDebtTokenDeposits()
				const TRIinSP_BeforeERC20 = await stabilityPool.getTotalDebtTokenDeposits()
				...
				// Check TRI balances of A, B, C have risen by the value of their compounded deposits, respectively
				const alice_expectedTRIBalance = alice_TRI_Balance_Before.add(alice_Deposit_Before).toString()
				const bob_expectedTRIBalance = bob_TRI_Balance_Before.add(bob_Deposit_Before).toString()
				const carol_expectedTRIBalance = carol_TRI_Balance_Before.add(carol_Deposit_Before).toString()

				const alice_TRI_Balance_After = (await debtToken.balanceOf(alice)).toString()
				const bob_TRI_Balance_After = (await debtToken.balanceOf(bob)).toString()
				const carol_TRI_Balance_After = (await debtToken.balanceOf(carol)).toString()

				assert.equal(alice_TRI_Balance_After, alice_expectedTRIBalance)
				assert.equal(bob_TRI_Balance_After, bob_expectedTRIBalance)
				assert.equal(carol_TRI_Balance_After, carol_expectedTRIBalance)
				...
				// Check TRI in Stability Pool has been reduced by A, B and C's compounded deposit
				const expectedTRIinSP = TRIinSP_Before.sub(alice_Deposit_Before)
